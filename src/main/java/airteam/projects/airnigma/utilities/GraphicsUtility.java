...
public class GraphicsUtility {
	...
	public static void drawFadedBorder(Graphics2D g, Color shadowColor, int shadowSize, int x, int y, int w, int h) {
		for (int i = 0; i < shadowSize; i++) {
			g.setColor(
					new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(), ((80 / shadowSize) * i)));
			g.drawRect(x + i + 1, y + i + 1, w - ((i * 2) + 1), h - ((i * 2) + 1));
		}
	}

	public static void drawRoundFadedBorder(Graphics2D g, Color shadowColor, int shadowSize, int x, int y, int w, int h,
			int borderRadius) {
		for (int i = 0; i < shadowSize; i++) {
			g.setColor(
					new Color(shadowColor.getRed(), shadowColor.getGreen(), shadowColor.getBlue(), ((80 / shadowSize) * i)));
	...
	public static BufferedImage getScaledImage(BufferedImage image, double scale) {
		BufferedImage tmpImage = new BufferedImage((int) Math.round(image.getWidth() * scale),
				(int) Math.round(image.getHeight() * scale), 2);
		tmpImage.getGraphics().drawImage(image.getScaledInstance((int) Math.round(image.getWidth() * scale),
				(int) Math.round(image.getHeight() * scale), Image.SCALE_SMOOTH), 0, 0, null);
	...
	public static BufferedImage getInternalIcon(String imagePath) {
		try {
			...
			LogUtility.logError("Wystapil blad przy pobieraniu ikonki: " + e.getMessage(),
					"Path do ikonki: " + "/airteam/projects/atarilogo/resources/" + imagePath);
		}
